...
class PlanLoss(Metric):
    """
    Violation rate the top K trajectories.
    """
    def __init__(self, args: Dict):
        self.k = args['k']
        self.name = 'plan_' + str(self.k)
        ...
        self.gamma = args['gamma'] if args is not None and 'gamma' in args.keys() else 1
    ...
    def compute(self, predictions: Dict, ground_truth: Union[Dict, torch.Tensor], safemap: torch.Tensor, ini_pose: torch.Tensor, gamma) -> torch.Tensor:
        ...
        # _, inds_topk = torch.topk(log_probs, min_k, dim=1) # torch.Size([32, 10])
        # batch_inds = torch.arange(batch_size).unsqueeze(1).repeat(1, min_k) # torch.Size([32, 10])
        # traj_topk = traj_pred[batch_inds, inds_topk] # torch.Size([32, 10, 12, 2])
        """
