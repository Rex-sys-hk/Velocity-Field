...
import numpy as np
import sys
sys.path.append('utils/data_augmentation')
from kinematic_agent_augmentation import KinematicAgentAugmentor
from nuplan_utils.agents import Agents
from nuplan_utils.trajectory import Trajectory
...
class TestKinematicAgentAugmentation(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        plt.scatter(self.features['agents'].ego[0][:, 0], self.features['agents'].ego[0][:, 1], label='original')
        plt.scatter(self.targets['trajectory'].data[:, 0], self.targets['trajectory'].data[:, 1], label='original')
    ...
    def test_gaussian_augment(self) -> None:
        ...
        plt.plot(aug_feature['agents'].ego[0][:, 0], aug_feature['agents'].ego[0][:, 1], 'r', label='gau_augmented')
        plt.plot(aug_targets['trajectory'].data[:, 0], aug_targets['trajectory'].data[:, 1], 'r', label='gau_augmented')
        plt.axis('equal')
        plt.legend()
        plt.show()
    ...
    def test_uniform_augment(self) -> None:
        ...
        plt.plot(aug_feature['agents'].ego[0][:, 0], aug_feature['agents'].ego[0][:, 1], 'r', label='uni_augmented')
        plt.plot(aug_targets['trajectory'].data[:, 0], aug_targets['trajectory'].data[:, 1], 'r', label='uni_augmented')
        plt.axis('equal')
        plt.legend()
        plt.show()
    ...
    def test_no_augment(self) -> None:
        ...
        plt.plot(aug_feature['agents'].ego[0][:, 0], aug_feature['agents'].ego[0][:, 1], 'r', label='no_augmented')
        plt.plot(aug_targets['trajectory'].data[:, 0], aug_targets['trajectory'].data[:, 1], 'r', label='no_augmented')
        plt.axis('equal')
        plt.legend()
        plt.show()
