import logging
from typing import List, Optional, Tuple, cast, TypeVar, Any
...
class KinematicHistoryGenericAgentAugmentor(AbstractAugmentor):
    ...
    * minimize dist(perturbed_trajectory, ground_truth_trajectory)
    ...
    def safety_check(self, ego, all_agents: List[np.ndarray]) -> bool:
        """
    ...
    def augment(
        ...
        for batch_idx in range(len(features['generic_agents'].ego)):
            ...
            ego_trajectory = np.copy(features['generic_agents'].ego[batch_idx])
