...
Eigen::VectorXd Spline2d::get_splines() const {
  const int num_params = spline_order_ + 1;
  const int num_seg = t_knots_.size() - 1;
  const uint32_t offset = num_seg * num_params;

  Eigen::VectorXd params(2 * num_seg * num_params);

  for (uint32_t i = 0; i < splines_.size(); ++i) {
    auto spline_piece_x = splines_[i].spline_func_x().params();
    auto spline_piece_y = splines_[i].spline_func_y().params();

    for (uint32_t j = 0; j < num_params; ++j) {
      params(i * num_params + j) = spline_piece_x[j];
      params(i * num_params + offset + j) = spline_piece_y[j];
    }
  }
